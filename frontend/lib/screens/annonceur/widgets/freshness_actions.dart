...
class _FreshnessActionsState extends State<FreshnessActions> {
  ...
  Widget build(BuildContext context) {
    final confirmTap = _busy ? null : () => _run(() => widget.repository.confirmAvailable(widget.listing.id));
    final rentedTap = _busy ? null : () => _run(() => widget.repository.markRented(widget.listing.id));

    if (widget.compact) {
      return Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Tooltip(message: 'Toujours disponible', child: _iconOnlyButton('check', confirmTap)),
          const SizedBox(width: 6),
          Tooltip(message: 'Marquer louée', child: _iconOnlyButton('close', rentedTap)),
        ],
      );
    }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(child: _actionButton(icon: 'check', label: 'Toujours disponible', onTap: confirmTap)),
          const SizedBox(width: 8),
          Expanded(child: _actionButton(icon: 'close', label: 'Marquer louée', onTap: rentedTap)),
        ],
      ),
    );
  }
