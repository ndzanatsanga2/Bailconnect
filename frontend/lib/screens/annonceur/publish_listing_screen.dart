...
class _PublishListingScreenState extends State<PublishListingScreen> {
  ...
  Future<void> _submit() async {
    if (_titleController.text.trim().isEmpty || _neighborhoodController.text.trim().isEmpty) {
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const Text('Nouveau bien', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
                    ],
                  ),
                  const Text('1/1', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.sub)),
                ],
              ),
            ),
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      child: Container(
                        ...
                        child: const Center(
                          child: Column(
                            ...
                            children: [
                              BcIcon('images', size: 28, color: AppColors.greenDark),
                              SizedBox(height: 8),
                              Text('Ajouter vidéo 60 s + photos', style: TextStyle(color: AppColors.greenDark, fontWeight: FontWeight.w800, fontSize: 13)),
                            ],
                          ),
                        ),
                      ),
                      ...
                      SizedBox(
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) => ClipRRect(
                            ...
                            child: Image.memory(_pickedMedia[index].$2, width: 52, height: 52, fit: BoxFit.cover),
                          ),
                        ),
                      ),
                    ],
                    ...
                    _label('Titre & type'),
                    _textField(_titleController, hint: 'Studio meublé · Yaoundé'),
                      ...
                      children: [
                        for (final type in _propertyTypes)
                          BcChip(
                            ...
                            onTap: () => setState(() => _propertyType = type.$1),
                          ),
                      ],
                    ),
                    const SizedBox(height: 14),
                    _label('Localisation'),
                    _textField(_neighborhoodController, hint: 'Quartier (ex. Bastos)', icon: 'pin'),
                    const SizedBox(height: 9),
                    Row(
                      children: [
                        Expanded(child: _textField(_rentController, hint: 'Loyer /mois', keyboardType: TextInputType.number)),
                        const SizedBox(width: 8),
                        Expanded(child: _textField(_depositController, hint: 'Caution', keyboardType: TextInputType.number)),
                      ],
                    ),
                    const SizedBox(height: 9),
                    _textField(_whatsappController, hint: 'WhatsApp de contact (+237)', icon: 'phone'),
            ...
            BcStickyBar(
              child: SizedBox(
                ...
                child: FilledButton.icon(
                  ...
                  label: Text(
                    _submitting ? 'Publication...' : 'Publier — soumis à validation',
  ...
  Widget _textField(TextEditingController controller, {required String hint, String? icon, TextInputType? keyboardType}) {
    return TextField(
      controller: controller,
      keyboardType: keyboardType,
      decoration: InputDecoration(
        hintText: hint,
        prefixIcon: icon != null ? Padding(padding: const EdgeInsets.all(12), child: BcIcon(icon, size: 16, color: AppColors.sub)) : null,
